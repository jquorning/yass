...
package body Modules is
   ...
   is
      ...
      procedure Run_Module (Item : Directory_Entry_Type);
      ...
      is
         ...
         with Default_Value => NOTAG;

         Empty_Tag : constant Tag_Types := NOTAG;

         function Tag_Exist return Tag_Types;
         --  Check if tag with selected name exists and return it type

         procedure Send_Table_Tag (Table_Tags : TableTags_Container.Map);
         --  Send to the module values for selected composite tag in
         --  Table_Tags list of tags.
         --  First response contains amount of values.

         procedure Edit_Tag (Tags : in out Tags_Container.Map);
         --  Edit selected simple tag in selected Tags list of tags.

         procedure Edit_Table_Tag (Table_Tags  : in out TableTags_Container.Map);
         --  Edit selected composite tag in selected Table_Tags list of
         --   composite tags.
         ...
         function Tag_Exist return Tag_Types is
            use Tags_Container;
         begin
            if Site_Tags.Contains (Key => To_String (Tag_Name)) then
               return GLOBALTAG;

            elsif Global_Table_Tags.Contains (Key => To_String (Tag_Name)) then
            ...
            if Page_Tags.Contains (Key => To_String (Tag_Name)) then
               return PAGETAG;

            elsif Page_Table_Tags.Contains (Key => To_String (Tag_Name)) then
               return PAGETABLETAG;
            end if;
         ...
         begin
            ...
            for I in 1 .. Size (T => Table_Tags (Key)) loop
               Send
                 (Descriptor => Module,
                  Str        => AWS.Templates.Item (T => Table_Tags (Key),
                                                    N => I));
            ...
            Tags.Include
              (Key      => Key,
               New_Item =>
                 Slice
                   (Source => Text,
                    Low    => Index (Source  => Text,
                                     Pattern => " ",
                                     From    => 10) + 1,
                    High => Length (Text)));

            Send (Descriptor => Module,
                  Str        => "Success");
         end Edit_Tag;
         ...
         procedure Edit_Table_Tag
           (Table_Tags  : in out TableTags_Container.Map)
         is
            Start_Index : Positive;
            Tag_Value   : Unbounded_String;
            Tag_Index   : Unbounded_String;
         ...
         begin
              ...
              Unbounded_Slice
                (Source => Text,
                 Low    => Index (Source  => Text,
                                  Pattern => " ",
                                  From    => Start_Index) + 1,
                 High   => Index (Source  => Text,
                                  Pattern => " ",
                                  From => Start_Index + 1) - 1);
            ...
            if Table_Index <=
              Size (T => Table_Tags (To_String (Tag_Name))) and
              Table_Index > 0
            then
               Temp_Tag := +"";
               ...
               Set_Tags_Loop :
               for I in
                 1 .. Size (T => Table_Tags (To_String (Tag_Name)))
               loop
                  if Table_Index = I then
                     ...
                     Temp_Tag :=
                       Temp_Tag &
                       AWS.Templates.Item
                         (T => Table_Tags (To_String (Tag_Name)),
                          N => I);
                  end if;
            ...
            --  Invalid tag index in tags list
               ...
               Send
                 (Descriptor => Module,
                  Str        =>
                    "Index """ & To_String (Tag_Index) &
                    """ is not in tag """ & To_String (Tag_Name) &
                    """ index range.");
            end if;
         end Edit_Table_Tag;
            ...
            if Slice (Source => Text, Low => 1, High => 6) = "gettag" then
               ...
               case Tag_Exist is
                  ...
                  when GLOBALTAG =>
                     Send
                       (Descriptor => Module,
                        Str        => Site_Tags (To_String (Tag_Name)));
                  ...
                  when PAGETAG =>
                     Send
                       (Descriptor => Module,
                        Str        => Page_Tags (To_String (Tag_Name)));
